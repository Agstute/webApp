...
<script>	
function getSettings(relay_name_id, relay_id, control_id, sensor_id, timer_id, program_id, schedule_id) { //, relay_id, control_id, sensor_id, timer_id, program_id, schedule_id) {
...
$program="program";
$schedule="schedule";
echo "<span class='span_left'>Relay Name</span><input id='relay_name_".$k."' type='text' class='relay_name' value='".$a=fetch_data($table, $k, $relay_name)."'/><br/>";

echo "<br/><br/><br/>";
echo "<input id='control_".$k."' type='text' value='".$a=fetch_data($table, $k, $control)."' style='float: left; width: 20%; height:7%; background: beige'/><input  id='relay_".$k."' type='text' value='".$a=fetch_data($table, $k, $relay)."' style='float: right; width: 30%; height:7%; background: beige' /><br/>";
echo "<br/><br/><br/>";
echo "<span style='text-align: left;'>Sensor</span><input  id='sensor_".$k."' type='text'  value='".$a=fetch_data($table, $k, $sensor)."' style='float: right; width: 10%; height: auto; background: beige' />";
echo "<br/>";
echo "<span style='text-align: left;'>Timer</span><input id='timer_".$k."' type='text' value='".$a=fetch_data($table, $k, $timer)."' style='float: right; width: 10%; height: auto; background: beige' />";
echo "<br/><br/><br/><br/>";
echo "<span style='float: left'>Program</span> <input id='program_".$k."' type='text' value='".$a=fetch_data($table, $k, $program)."' style='float: right; width: 70%; height:7%; background: beige'/>";
echo "<br/><br/><br/>";
echo "<input id='schedule_".$k."' type='text' value='".$a=fetch_data($table, $k, $schedule)."' style='float: left; width: 40%; height:7%; background: beige'/>";
echo "<br/><br/><br/>";
//echo "<div style='width: 100%; height:auto;background: grey'>";
//put submit button here
echo "<button onclick='getSettings(`relay_name_".$k."`, `relay_".$k."`, `control_".$k."`, `sensor_".$k."`, `timer_".$k."`, `program_".$k."`,  `schedule_".$k."`)'>Set</button>";
...
//-----------//	
	...
	insert_2_db($table, $sensor, $threshold, $relay_name, $relay, $relay_on, $relay_off, $timer_name, $timer, $timer_on, $timer_off, $control, $program, $schedule, $k);
